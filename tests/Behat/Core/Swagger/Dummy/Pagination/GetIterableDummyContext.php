...
{
    /**
     * @Then get dummy iterable should be configured
     */
    public function then(): void
    {
        $response = $this->getResponse()->toArray();

        Assert::assertNotEmpty($response['paths']['/dummies_iterable']['get']);
        $getDummiesArrayResponse = $response['paths']['/dummies_iterable']['get'];

        Assert::assertArrayHasKey('summary', $getDummiesArrayResponse);
        Assert::assertArrayHasKey('description', $getDummiesArrayResponse);
        Assert::assertSame(
            'Retrieve a Dummy iterable',
